...
class DeferredPointerHandler extends StatefulWidget {
  const DeferredPointerHandler({Key? key, required this.child, this.link}) : super(key: key);
  final Widget child;
...
class _DeferredHitTargetRenderObjectWidget extends SingleChildRenderObjectWidget {
  const _DeferredHitTargetRenderObjectWidget({required this.link, Widget? child, Key? key})
  ...
  @override
  void updateRenderObject(BuildContext context, _DeferredHitTargetRenderObject renderObject) =>
...
class _InheritedDeferredPaintSurface extends InheritedWidget {
  const _InheritedDeferredPaintSurface({Key? key, required Widget child, required this.state})
